...
-- NEW: Current target coin and time tracking for auto-updates
rt.currentTargetCoin = nil :: Instance
rt.targetUpdateTime = 0 :: number
rt.targetUpdateInterval = 0.5 :: number -- Update target every 0.5 seconds
...
local function FindBestCoinTarget()
    if not rt.Murderer or not rt.Murderer.Character then return nil end
    
    local murdererPos = rt.Murderer.Character:FindFirstChild("HumanoidRootPart")
    if not murdererPos then return nil end
    
    local allCoins = rt.octree:GetNearest(rt:Character().PrimaryPart.Position, rt.radius, math.huge)
    local bestCoin = nil
    local bestDistance = -1
    
    for _, node in pairs(allCoins or {}) do
        local coin = node.Object
        if coin and not isCoinTouched(coin) and coin.Parent then
            local distFromMurderer = (coin.Position - murdererPos.Position).Magnitude
            if distFromMurderer > bestDistance then
                bestDistance = distFromMurderer
                bestCoin = coin
            end
        end
    end
...
collectCoins = function ()
    ...
    while rt.AutoFarmOn do
        ...
        -- NEW: Auto-update target coin periodically
        if tick() - rt.targetUpdateTime >= rt.targetUpdateInterval then
            rt.targetUpdateTime = tick()
            
            -- Check if current target is still valid
            if rt.currentTargetCoin and (isCoinTouched(rt.currentTargetCoin) or not rt.currentTargetCoin.Parent) then
                rt.currentTargetCoin = nil
            end
            
            -- Find new target if none exists
            if not rt.currentTargetCoin then
                rt.currentTargetCoin = FindBestCoinTarget()
            end
        end

        if rt.currentTargetCoin and rt.currentTargetCoin.Parent then
            local targetCoinPosition = rt.currentTargetCoin.Position
            ...
            markCoinAsTouched(rt.currentTargetCoin)
            rt.currentTargetCoin = nil
            task.wait(0.2) -- Ensure touch is registered
        else
            task.wait(0.5) -- No valid coins; retry after delay
        end
    end
